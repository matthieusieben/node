...
class FilesWatcher extends EventEmitter {
  ...
  #onChange(trigger) {
    clearTimeout(this.#debouncing.get(trigger));
    ...
    this.#debouncing.set(trigger, timer);

    clearTimeout(this.#globalChangesDebouncing);
    this.#globalChangesDebouncing = setTimeout(() => {
      this.#globalChangesDebouncing = null;
      this.emit('restart');
    }, this.#debounce).unref();
  }
